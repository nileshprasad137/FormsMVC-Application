<?php /* Smarty version 2.6.29, created on 2017-03-06 18:41:57
         compiled from default/views/month/ajax_template.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('function', 'config_load', 'default/views/month/ajax_template.html', 10, false),array('modifier', 'date_format', 'default/views/month/ajax_template.html', 331, false),array('modifier', 'string_format', 'default/views/month/ajax_template.html', 332, false),)), $this); ?>
...
<a href='<?php echo $PREV_MONTH_URL ?>' onclick='top.restoreSession()'>
<img id="prevmonth" src="<?php echo $this->_tpl_vars['TPL_IMAGE_PATH']; ?>
/leftbtn.gif" border="0" title="<?php echo xl("Previous Month") ?>" alt="<?php echo xl ("Previous Month") ?>" style="padding-top:5px"/></a>
<a href='<?php echo $NEXT_MONTH_URL ?>' onclick='top.restoreSession()'>
<img id="nextmonth" src="<?php echo $this->_tpl_vars['TPL_IMAGE_PATH']; ?>
/rightbtn.gif" border="0" title="<?php echo xl("Next Month") ?>" alt="<?php echo xl ("Next Month") ?>" /></a>
&nbsp;
<?php 
...
// ==============================
 echo "</div>";
 echo "   <select multiple size='15' name='pc_username[]' id='pc_username' class='view2'>\n";
...
//=======================================================================================
        foreach ($events as $event) {
            ...
            $eventid = $event['eid'];
            $eventtype = sqlQuery("SELECT pc_cattype FROM libreehr_postcalendar_categories as oc LEFT OUTER JOIN libreehr_postcalendar_events as oe ON oe.pc_catid=oc.pc_catid WHERE oe.pc_eid='".$eventid."'");
            $pccattype = '';
        ...
        $fname = substr($event['patient_name'], $commapos + 2);
            ...
            if (date("i", $startDateTime) == "00") {
                ...
                $displayTime .= (date("a", $startDateTime));
                   ...
                   $displayTime .= (date(":i", $startDateTime));
            }
            }
            else {
                if ( $GLOBALS['time_display_format'] == 1) {
                $displayTime .= (date(":ia", $startDateTime));
                   ...
                   $displayTime = date("H", $startDateTime);
                   $displayTime .= (date(":i", $startDateTime));
                }
            }
            ...
            $result = sqlStatement("SELECT name,id,color FROM facility WHERE id=(SELECT pc_facility FROM libreehr_postcalendar_events WHERE pc_eid=".$eventid.")");
